...
class CRM_Cdntaxreceipts_Task_IssueAggregateTaxReceipts extends CRM_Contribute_Form_Task {
  ...
  function preProcess() {
    ...
    $receipts = array('totals' =>
      array(
        ...
        'original' => 0,
        'duplicate' => 0,
        'ineligibles' => 0 // CH Customization: include ineligibles in list, but won't be processed
      ),
    );
    ...
    foreach ($this->_contributionIds as $id) {
      ...
      if (is_array($status)) {
        ...
        // CH Customization: check if most recent is cancelled, and mark as "replace" then add that contribution to 'original' receipt array
  ...
  function postProcess() {
    ...
    foreach ($this->_receipts['original'][$year]['contact_ids'] as $contact_id => $contribution_status) {
      ...
      // CH Customization: CRM-1470 Create separate In Kind contributions array and unset from combined tax receipt contributions array
      ...
      if ( empty($issuedOn) && count($contributions) > 0 ) {
        ...
        list($success, $method) = cdntaxreceipts_issueAggregateTaxReceipt($contact_id, $year, $contributions, $method,
          $receiptsForPrintingPDF, $previewMode, $thankyou_html);

        if ( $success && !$previewMode ) {
            // CH Customization: CRM-920: Mark Contribution as thanked if checked
      ...
      // CH Customization: CRM-1470 Generate individual In Kind contributions receipts
      foreach ($contributionsInKind as $inkind_key => $inkind_value) {
        ...
        if ( cdntaxreceipts_eligibleForReceipt($contribution->id) ) {
          ...
          // CH Customization: CRM-1990-Receipt not getting replaced for a cancelled In Kind donation through Aggregate Tax Receipt method
  ...
  ***************************************/
  ...
  private function getThankYouHTML(array $contributionIds, $sender, $params) {
    ...
    $data['values']['IssueAggregateTaxReceipts']['from_email_address'] = $sender;
    $data['values']['IssueAggregateTaxReceipts']['subject'] = $this->getElement('subject')->getValue();
    $data['values']['IssueAggregateTaxReceipts']['html_message'] = $this->getElement($html_message)->getValue();
    //CRM-1792 Adding 'group_by' parameter for token processor to process grouped contributions
    if (count($contributionIds) > 1) {
      $data['values']['IssueAggregateTaxReceipts']['group_by'] = 'contact_id';
    }
